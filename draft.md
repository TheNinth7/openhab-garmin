...
Built using the Garmin Connect IQ SDK, the app is based on [openHAB sitemaps](https://www.openhab.org/docs/ui/sitemaps.html).

It includes the following application types:

- **Glance**: the entry point into the widget, showing the label of the sitemap. 
- **Widget**: The main interface, build based on the SDK's `CustomMenu` and `CustomMenuItem`.
